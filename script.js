...
function showLoadingIndicator() {
  ...
  resultsDiv.innerHTML =
    '<div class="loading"><div class="spinner"></div>Loading ballot data...</div>';
}
      ...
      matchingZips.map(async (match) => {
        try {
          ...
          results = response.data.filter(
            (row) => row.zip && row.zip.includes(match.zip)
          );
          return results;
        } catch (error) {
          console.error(
            `Error loading ballot data for zip ${match.zip}:`,
            error
          );
          return [];
        }
  ...
  if (results.length > 0) {
                    ...
                    <div id="ballot-${index}" class="ballot-content" style="display: none;">
                        <div class="ballot-tabs">
                            <div class="tab-buttons">
                                <button class="tab-button active" onclick="switchTab(${index}, 'simplified')">Short Ballot</button>
    ...
    if (results.length === 1) {
      ...
      const matchingResults = results
        ...
        .filter(
          (item) =>
            item.result.county.toLowerCase() + " county" ===
              (county || "").toLowerCase() && item.result.zip === zip
        );
...
function toggleBallot(index) {
  ...
  ballotContent.style.display =
    ballotContent.style.display === "none" ? "block" : "none";

  // Update URL with toggle states
  const toggleStates = Array.from(
    document.querySelectorAll(".ballot-content")
  ).map((content) => (content.style.display === "none" ? "0" : "1"));
